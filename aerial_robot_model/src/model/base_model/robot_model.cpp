...
namespace aerial_robot_model {
  ...
  RobotModel::RobotModel(bool init_with_rosparam, bool verbose, bool fixed_model, double fc_f_min_thre, double fc_t_min_thre, double epsilon):
    ...
    initialized_(false)
  {
    // NOTE: ROS2 migration: getParamFromRos() and kinematicsInit() now require a node, pass node explicitly in your application.
    gravity_.resize(6);
    gravity_ <<  0, 0, 9.80665, 0, 0, 0;
    gravity_3d_.resize(3);
    gravity_3d_ << 0, 0, 9.80665;
    // kinematicsInit(), stabilityInit(), staticsInit() must be called with a node from outside in ROS2.
  }

  void RobotModel::getParamFromRos(const rclcpp::Node::SharedPtr& node)
  {
    node->declare_parameter("kinematic_verbose", false);
    node->declare_parameter("fc_f_min_thre", 0.0);
    node->declare_parameter("fc_t_min_thre", 0.0);
    node->declare_parameter("epsilon", 10.0);
    node->get_parameter("kinematic_verbose", verbose_);
    node->get_parameter("fc_f_min_thre", fc_f_min_thre_);
    node->get_parameter("fc_t_min_thre", fc_t_min_thre_);
    node->get_parameter("epsilon", epsilon_);
  }

  void RobotModel::kinematicsInit(const rclcpp::Node::SharedPtr& node)
  {
    ...
    if (!model_.initString(urdf_string)) {
      RCLCPP_ERROR(node->get_logger(), "Failed to extract urdf model from robot_description parameter");
      return;
    }
    ...
    // Use default baselink and thrust_link, or optionally declare as parameters
    node->declare_parameter("baselink", baselink_);
    node->declare_parameter("thrust_link", thrust_link_);
    node->get_parameter("baselink", baselink_);
    node->get_parameter("thrust_link", thrust_link_);

    if(!model_.getLink(baselink_)) {
      RCLCPP_ERROR(node->get_logger(), "Can not find the link named '%s' in urdf model", baselink_.c_str());
      return;
    }
    ...
    for(const auto& link: urdf_links) {
      if(link->name.find(thrust_link_) != std::string::npos)
        found_thrust_link = true;
    }
    if(!found_thrust_link) {
      RCLCPP_ERROR(node->get_logger(), "Can not find the link named '%s' in urdf model", thrust_link_.c_str());
      return;
    }
  ...
  void RobotModel::staticsInit(const rclcpp::Node::SharedPtr& node)
  {
    // Optionally get m_f_rate from parameter
    node->declare_parameter("m_f_rate", 0.0);
    node->get_parameter("m_f_rate", m_f_rate_);
  ...
  KDL::RigidBodyInertia RobotModel::inertialSetup(const KDL::TreeElement& tree_element)
  {
    ...
    if(verbose_) RCLCPP_WARN_STREAM(rclcpp::get_logger("aerial_robot_model"), "segment " <<  current_seg.getName() << ", mass is: " << current_seg_inertia.getMass());
    ...
    if(current_seg.getName().find("root") != std::string::npos)
      {
        ...
        if(verbose_) RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "Add root link: %s", child_seg.getName().c_str());
        ...
        if(current_seg.getJoint().getName().find("rotor") == std::string::npos)
          {
            ...
            if(verbose_) RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "Add new inertia base link: %s", current_seg.getName().c_str());
          }
      }
    ...
    if(current_seg.getJoint().getName().find("rotor") != std::string::npos)
      {
        ...
        if(urdf_joint->type == urdf::Joint::CONTINUOUS)
          {
            if(verbose_) RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "joint name: %s, z axis: %f", current_seg.getJoint().getName().c_str(), urdf_joint->axis.z);
    ...
    for (const auto& elem: GetTreeElementChildren(tree_element))
      {
        ...
        if(verbose_) RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "Add new child segment %s to direct segment: %s", child_seg.getName().c_str(), current_seg.getName().c_str());
      }
    ...
    if (inertia_map_.find(current_seg.getName()) != inertia_map_.end())
      {
        ...
        if(verbose_) RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "Total mass of base segment %s is %f", current_seg.getName().c_str(),
    ...
    if(extra_module_map_.find(module_name) == extra_module_map_.end())
      {
        if(inertia_map_.find(parent_link_name) == inertia_map_.end())
          {
            RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "[extra module]: fail to add new extra module %s, because its parent link (%s) does not exist", module_name.c_str(), parent_link_name.c_str());
            return false;
          }

        if(!aerial_robot_model::isValidRotation(transform.M))
          {
            RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "[extra module]: fail to add new extra module %s, because its orientation is invalid", module_name.c_str());
            return false;
          }

        if(inertia.getMass() <= 0)
          {
            RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "[extra module]: fail to add new extra module %s, becuase its mass %f is invalid", module_name.c_str(), inertia.getMass());
            return false;
          }
        ...
        RCLCPP_INFO(rclcpp::get_logger("aerial_robot_model"), "[extra module]: succeed to add new extra module %s", module_name.c_str());
      ...
      {
        RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "[extra module]: fail to add new extra module %s, becuase it already exists", module_name.c_str());
        return false;
      }
  }
  ...
  bool RobotModel::removeExtraModule(std::string module_name)
  {
    ...
    if(it == extra_module_map_.end())
      {
        RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), "[extra module]: fail to remove the extra module %s, because it does not exists", module_name.c_str());
        return false;
      }
    else
      {
        extra_module_map_.erase(module_name);
        RCLCPP_INFO(rclcpp::get_logger("aerial_robot_model"), "[extra module]: succeed to remove the extra module %s", module_name.c_str());
  ...
  void RobotModel::updateRobotModel(const sensor_msgs::msg::JointState& state)
  {
  ...
  void RobotModel::updateRobotModelImpl(const KDL::JntArray& joint_positions)
  {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("aerial_robot_model"), "[aerial_robot_model] robot mass is " << mass_);
    ...
    for(int i = 0; i < rotor_num_; ++i)
      {
        ...
        if(verbose_) RCLCPP_WARN(rclcpp::get_logger("aerial_robot_model"), " %s : [%f, %f, %f]", rotor.c_str(), f.p.x(), f.p.y(), f.p.z());
  ...
  KDL::Frame RobotModel::forwardKinematicsImpl(std::string link, const KDL::JntArray& joint_positions) const
  {
    ...
    if(status < 0) RCLCPP_ERROR(rclcpp::get_logger("aerial_robot_model"), "can not solve FK to link: %s", link.c_str());
  ...
  bool RobotModel::stabilityCheck(bool verbose)
  {
    if(fc_f_min_ < fc_f_min_thre_)
      {
        if(verbose)
          RCLCPP_ERROR_STREAM(rclcpp::get_logger("aerial_robot_model"), "the min distance to the plane of feasible control force convex " << fc_f_min_ << " is lower than the threshold " <<  fc_f_min_thre_);
          return false;
      }
    ...
    if(fc_t_min_ < fc_t_min_thre_)
      {
        if(verbose)
          RCLCPP_ERROR_STREAM(rclcpp::get_logger("aerial_robot_model"), "the min distance to the plane of feasible control torque convex " << fc_t_min_ << " is lower than the threshold " <<  fc_t_min_thre_);
        return false;
      }
    ...
    if(static_thrust_.maxCoeff() > thrust_max_ || static_thrust_.minCoeff() < thrust_min_)
      {
        if(verbose)
          RCLCPP_ERROR(rclcpp::get_logger("aerial_robot_model"), "Invalid static thrust, max: %f, min: %f", static_thrust_.maxCoeff(), static_thrust_.minCoeff());
        return false;
      }
  ...
  std::string RobotModel::getRobotModelXml(const std::string& param, const rclcpp::Node::SharedPtr& node)
  {
    if (!node) {
      throw std::runtime_error("getRobotModelXml: node is nullptr");
    }
    std::string urdf_string;
    if (!node->has_parameter(param)) {
      throw std::runtime_error("getRobotModelXml: parameter '" + param + "' not declared on node");
    }
    node->get_parameter(param, urdf_string);
    if (urdf_string.empty()) {
      throw std::runtime_error("getRobotModelXml: parameter '" + param + "' is empty");
    }
    return urdf_string;
  }
